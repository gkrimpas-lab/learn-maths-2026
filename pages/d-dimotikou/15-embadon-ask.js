// pages/d-dimotikou/15-embadon-ask.js
...
// 1. Μετατροπή από Μεγάλη σε Μικρότερη Μονάδα (Input)
function makeBigToSmallQuestion(prevQuestion = null) {
  const types = [
    { from: 'τ.μ.', to: 'τ.δ.', factor: 100, min: 2, max: 25 },
    { from: 'τ.δ.', to: 'τ.εκ.', factor: 100, min: 2, max: 30 },
    { from: 'τ.μ.', to: 'τ.εκ.', factor: 10000, min: 1, max: 8 }
  ];
...
// 2. Μετατροπή από Μικρή σε Μεγαλύτερη Μονάδα (Input)
function makeSmallToBigQuestion(prevQuestion = null) {
  const types = [
    { from: 'τ.δ.', to: 'τ.μ.', factor: 100, min: 2, max: 20 },
    { from: 'τ.εκ.', to: 'τ.δ.', factor: 100, min: 2, max: 30 },
    { from: 'τ.εκ.', to: 'τ.μ.', factor: 10000, min: 1, max: 8 }
  ];
...
// 3. Κατάλληλη Μονάδα Εμβαδού (ΟΜΑΔΑ Α - 4 Επιλογές MCQ)
const SUITABLE_UNITS_POOL = [
  {
    q: 'το εμβαδόν του δαπέδου μιας σχολικής αίθουσας',
    correct: 'Τετραγωνικά μέτρα (τ.μ.)',
    contextDesc: 'επιφάνειες δωματίων'
  },
  {
    q: 'το εμβαδόν μιας σελίδας ενός σχολικού τετραδίου',
    correct: 'Τετραγωνικά εκατοστά (τ.εκ.)',
    contextDesc: 'επιφάνειες βιβλίων και χαρτιών'
  },
  {
    q: 'το εμβαδόν ενός γηπέδου ποδοσφαίρου',
    correct: 'Τετραγωνικά μέτρα (τ.μ.)',
    contextDesc: 'μεγάλους αθλητικούς χώρους'
  },
  {
    q: 'το εμβαδόν της επιφάνειας μιας τραπεζικής κάρτας',
    correct: 'Τετραγωνικά εκατοστά (τ.εκ.)',
    contextDesc: 'μικρά επίπεδα αντικείμενα'
  },
  {
    q: 'το εμβαδόν της αυλής ενός σχολείου ή σπιτιού',
    correct: 'Τετραγωνικά μέτρα (τ.μ.)',
    contextDesc: 'οικόπεδα και αυλές'
  },
  {
    q: 'το εμβαδόν μιας τυπωμένης φωτογραφίας διάστασης 10 × 15 cm',
    correct: 'Τετραγωνικά εκατοστά (τ.εκ.)',
    contextDesc: 'μικρές φωτογραφικές επιφάνειες'
  },
  {
    q: 'το εμβαδόν της οθόνης ενός κινητού τηλεφώνου smartphone',
    correct: 'Τετραγωνικά εκατοστά (τ.εκ.)',
    contextDesc: 'επιφάνειες φορητών συσκευών'
  },
  {
    q: 'το εμβαδόν μιας πόλης ή ενός μεγάλου δάσους',
    correct: 'Τετραγωνικά χιλιόμετρα (τ.χλμ.)',
    contextDesc: 'τεράστιες γεωγραφικές εκτάσεις'
  }
];
...
function makeSuitableUnitQuestion(prevQuestion = null) {
  ...
  const allChoices = [
    ...
    'Τετραγωνικά δεκατόμετρα (τ.δ.)',
    'Τετραγωνικά χιλιόμετρα (τ.χλμ.)'
  ];
  ...
  return {
    ...
    explainText: `Για να μετρήσουμε ${selected.contextDesc}, η πλέον κατάλληλη μονάδα μέτρησης είναι τα ${selected.correct}.`
  };
}
...
function makeComparisonQuestion(prevQuestion = null) {
  const pairs = [
    { unitA: 'τ.μ.', unitB: 'τ.δ.', factorA: 100 },
    { unitA: 'τ.δ.', unitB: 'τ.εκ.', factorA: 100 },
    { unitA: 'τ.μ.', unitB: 'τ.εκ.', factorA: 10000 }
  ];
  ...
  while (true) {
    ...
    if (Math.random() > 0.35) {
      ...
      if (valB <= 0 || valB === valA * pair.factorA) {
        valB = valA * pair.factorA + (pair.factorA >= 1000 ? 500 : 50);
      }
    }
...
export default function EmbadonAskPage() {
      ...
      <div className={`bg-white p-5 sm:p-7 rounded-3xl shadow-sm border transition-all ${
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <h3 className="text-base sm:text-lg font-bold text-slate-900 leading-snug">
            Μετάτρεψε τη μονάδα εμβαδού: <span className="text-emerald-600 font-mono font-black text-lg sm:text-xl">{qData.val} {qData.from}</span> ＝ <span className="text-emerald-600 font-mono font-black text-lg sm:text-xl">? {qData.to}</span>
          </h3>
        </div>
    ...
    return (
      <div className={`bg-white p-5 sm:p-7 rounded-3xl shadow-sm border transition-all ${
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <h3 className="text-base sm:text-lg font-bold text-slate-900 leading-snug">
            Μετάτρεψε τη μονάδα εμβαδού: <span className="text-teal-600 font-mono font-black text-lg sm:text-xl">{formatNumber(qData.val)} {qData.from}</span> ＝ <span className="text-teal-600 font-mono font-black text-lg sm:text-xl">? {qData.to}</span>
          </h3>
        </div>
        ...
        {submitted && (
          <div className="mt-4 sm:pl-11 text-xs sm:text-sm leading-relaxed">
            {isCorrect ? (
              ...
              <p className="text-rose-700 font-medium bg-rose-50 p-2.5 rounded-xl border border-rose-200/60">
                Η καταλληλότερη μονάδα είναι: <strong className="font-bold text-rose-900">{qData.correct}</strong>. {qData.explainText}
              </p>
            )}
          </div>
        )}
        ...
        {submitted && (
          <div className="mt-4 sm:pl-11 text-xs sm:text-sm leading-relaxed">
            {isCorrect ? (
              ...
              <p className="text-rose-700 font-medium bg-rose-50 p-2.5 rounded-xl border border-rose-200/60">
                Η σωστή σχέση είναι <span className="font-mono font-bold text-rose-900">{qData.valA} {qData.unitA} {qData.correct} {formatNumber(qData.valB)} {qData.unitB}</span>. {qData.explainText}
              </p>
            )}
          </div>
        )}
  ...
  return (
    <Layout
      title="Ασκήσεις: Εμβαδόν & Μονάδες Μέτρησης | LearnMaths.gr"
      description="Διαδραστικές ασκήσεις μαθηματικών Δ' Δημοτικού στο εμβαδόν: μετατροπές τ.μ., τ.δ. και τ.εκ., επιλογή κατάλληλης μονάδας επιφάνειας και σύγκριση."
      backUrl="/d-dimotikou"
      backText="Δ' Δημοτικού"
      hideFooter={true}
      actionButton={
        <Link
          href="/d-dimotikou/15-embadon"
        ...
        <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600 text-white p-6 sm:p-8 rounded-3xl shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight pt-1">
              📝 Ασκήσεις: Μέτρηση Εμβαδού
            </h1>
            <p className="text-emerald-100 text-xs sm:text-sm md:text-base">
              Πατώντας «Νέες Ασκήσεις», οι αριθμοί και οι μονάδες ανανεώνονται αυτόματα από τη δεξαμενή!
            </p>
          </div>
