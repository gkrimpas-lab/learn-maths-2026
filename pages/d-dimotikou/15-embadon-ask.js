...
function makeBigToSmallQuestion(prevQuestion = null) {
  const types = [
    { from: 'τ.μ.', to: 'τ.δ.', factor: 100, min: 1, max: 15 },
    { from: 'τ.δ.', to: 'τ.εκ.', factor: 100, min: 1, max: 20 },
...
function makeSmallToBigQuestion(prevQuestion = null) {
  const types = [
    { from: 'τ.δ.', to: 'τ.μ.', factor: 100, min: 1, max: 12 },
    { from: 'τ.εκ.', to: 'τ.δ.', factor: 100, min: 1, max: 20 },
...
// 3. Κατάλληλη Μονάδα Εμβαδού (MCQ) - Pool
const SUITABLE_UNITS_POOL = [
  { q: 'Το εμβαδόν του δαπέδου μιας σχολικής αίθουσας', correct: 'Τετραγωνικά μέτρα (τ.μ.)', wrongs: ['Τετραγωνικά εκατοστά (τ.εκ.)', 'Τετραγωνικά χιλιοστά (τ.χιλ.)'] },
  { q: 'Το εμβαδόν μιας σελίδας τετραδίου', correct: 'Τετραγωνικά εκατοστά (τ.εκ.)', wrongs: ['Τετραγωνικά μέτρα (τ.μ.)', 'Τετραγωνικά χιλιόμετρα (τ.χλμ.)'] },
  { q: 'Το εμβαδόν ενός γηπέδου ποδοσφαίρου', correct: 'Τετραγωνικά μέτρα (τ.μ.)', wrongs: ['Τετραγωνικά εκατοστά (τ.εκ.)', 'Τετραγωνικά δεκατόμετρα (τ.δ.)'] },
  { q: 'Το εμβαδόν μιας πιστωτικής κάρτας', correct: 'Τετραγωνικά εκατοστά (τ.εκ.)', wrongs: ['Τετραγωνικά μέτρα (τ.μ.)', 'Τετραγωνικά χιλιόμετρα (τ.χλμ.)'] },
  { q: 'Το εμβαδόν μιας αυλής σπιτιού', correct: 'Τετραγωνικά μέτρα (τ.μ.)', wrongs: ['Τετραγωνικά εκατοστά (τ.εκ.)', 'Τετραγωνικά χιλιοστά (τ.χιλ.)'] },
  { q: 'Το εμβαδόν μιας φωτογραφίας 10x15 cm', correct: 'Τετραγωνικά εκατοστά (τ.εκ.)', wrongs: ['Τετραγωνικά μέτρα (τ.μ.)', 'Τετραγωνικά χιλιόμετρα (τ.χλμ.)'] }
];
...
export default function EmbadonAskPage() {
    ...
    <div className={`bg-white p-6 md:p-8 rounded-3xl shadow-sm border transition-all ${
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold text-gray-900">
          Μετάτρεψε τη μονάδα εμβαδού: <span className="text-emerald-600 font-mono font-black text-xl">{qData.val} {qData.from}</span> = <span className="text-emerald-600 font-mono font-black text-xl">? {qData.to}</span>
        </h3>
      </div>
  ...
  const renderSmallToBig = (qKey, qData, numLabel) => (
    <div className={`bg-white p-6 md:p-8 rounded-3xl shadow-sm border transition-all ${
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold text-gray-900">
          Μετάτρεψε τη μονάδα εμβαδού: <span className="text-teal-600 font-mono font-black text-xl">{qData.val.toLocaleString('el-GR')} {qData.from}</span> = <span className="text-teal-600 font-mono font-black text-xl">? {qData.to}</span>
        </h3>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans flex flex-col justify-between pb-24">
      <Head>
        <title>🟩 Ασκήσεις: Εμβαδόν & Μονάδες - LearnMaths.gr</title>
        ...
        <nav className="bg-white shadow-md w-full sticky top-0 z-50">
          <div className={`${LAYOUT.CONTAINER} py-4 flex justify-between items-center`}>
            ...
            <div className="flex items-center gap-3">
              <Link href="/d-dimotikou/15-embadon" className="bg-emerald-100 hover:bg-emerald-200 text-emerald-800 font-bold px-4 py-2.5 rounded-xl text-sm transition shadow-sm flex items-center gap-2">
                <span>📖</span> Θεωρία
              </Link>
              <button 
          ...
          <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600 text-white p-8 rounded-3xl shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-emerald-100 text-sm md:text-base mt-1">
                Πατώντας «Νέες Ασκήσεις» οι αριθμοί και οι ερωτήσεις αλλάζουν.
              </p>
            </div>
