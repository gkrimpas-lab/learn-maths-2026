...
// 1. Ασκηση: Μετατροπή Κλάσματος σε Δεκαδικό (Input)
function makeFractionToDecimalQuestion() {
  const isTenths = Math.random() > 0.5;
  const den = isTenths ? 10 : 100;
  const num = isTenths ? getRandomInt(1, 99) : getRandomInt(1, 299);
  const val = (num / den).toFixed(isTenths ? 1 : 2).replace('.', ',');
...
function makeDecimalToFractionQuestion() {
  const isTenths = Math.random() > 0.5;
  const den = isTenths ? 10 : 100;
  const num = isTenths ? getRandomInt(1, 9) : getRandomInt(1, 99);
  const decVal = (num / den).toFixed(isTenths ? 1 : 2).replace('.', ',');

  const correctText = `${num}/${den}`;
  const wrong1 = `${num}/${den === 10 ? 100 : 10}`;
  const wrong2 = `${num + 5}/${den}`;
  const wrong3 = `${den}/${num}`;
...
// 4. Ασκηση: Σύγκριση Δεκαδικών (Buttons <, =, >)
function makeComparisonQuestion() {
  const intPart = getRandomInt(1, 15);
  const decA = getRandomInt(1, 95);
  let decB = getRandomInt(1, 95);

  // Πιθανότητα να έχουν το ίδιο ακέραιο μέρος
  const valA = parseFloat(`${intPart}.${decA < 10 ? '0' + decA : decA}`);
  let valB = parseFloat(`${intPart}.${decB < 10 ? '0' + decB : decB}`);
  ...
  return {
    strA: valA.toString().replace('.', ','),
    strB: valB.toString().replace('.', ','),
    correct: correctSym
  };
}
...
export default function DekadikoiAskPage() {
  ...
  const handleSubmit = (e) => {
    ...
    // Καθαρισμός απαντήσεων με κόμμα/τελεία για Q1 & Q2
    ...
    <div className={`bg-white p-6 md:p-8 rounded-3xl shadow-sm border transition-all ${
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
          ...
          <span className="inline-flex flex-col items-center leading-none text-base font-mono font-black text-teal-700 bg-teal-50 px-2 py-1 rounded-lg border border-teal-200">
            <span>{qData.num}</span>
            <span className="border-b-2 border-teal-800 w-full"></span>
            <span>{qData.den}</span>
          </span> 
          ως δεκαδικό αριθμό:
        </h3>
      ...
      {submitted && (
        <div className="mt-4 pl-0 md:pl-11 text-xs md:text-sm font-bold">
          {answers[qKey].toString().trim().replace('.', ',') === qData.correct ? (
            ...
            <p className="text-red-600">❌ Λάθος. Ο σωστός δεκαδικός αριθμός είναι: <span className="font-mono font-black">{qData.correct}</span></p>
          )}
        </div>
      )}
    </div>
        ...
        <nav className="bg-white shadow-md w-full sticky top-0 z-50">
          <div className={`${LAYOUT.CONTAINER} py-4 flex justify-between items-center`}>
            ...
            <div className="flex items-center gap-3">
              <Link href="/d-dimotikou/7-dekadikoi-sinexeia" className="bg-teal-100 hover:bg-teal-200 text-teal-800 font-bold px-4 py-2.5 rounded-xl text-sm transition shadow-sm flex items-center gap-2">
                <span>📖</span> Θεωρία
              </Link>
              <button 
