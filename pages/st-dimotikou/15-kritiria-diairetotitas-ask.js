...
function generateQuestions() {
  ...
  // Q1: Input - Διαιρετότητα με το 2, 5 ή 10
  ...
  if (q1Div === 2) {
    ...
    if (q1Num % 10 === 0 && Math.random() > 0.5) q1Num += 5;
  } else {
    q1Num = Math.floor(q1Num / 10) * 10;
  }
  const q1Correct = String(q1Num % q1Div === 0 ? 'Ναι' : 'Όχι');
  const q1Prompt = `Διαιρείται ο αριθμός ${q1Num} ακριβώς με το ${q1Div}; (Γράψε Ναι ή Όχι)`;
  ...
  // Βρίσκουμε d2 ώστε (d1 + d2 + d3) % q4Div === 0
  let correctDigit = 0;
  ...
  return {
    q1: {
      type: 'input',
      title: 'Έλεγχος Διαιρετότητας',
      prompt: q1Prompt,
      number: String(q1Num),
      divisor: q1Div,
      correct: q1Correct,
      explain: q1Num % q1Div === 0
        ? `Σωστά! Το τελευταίο ψηφίο είναι ${q1Num % 10}, επομένως ο αριθμός ${q1Num} διαιρείται με το ${q1Div}.`
        : `Ο αριθμός ${q1Num} τελειώνει σε ${q1Num % 10}, άρα ΔΕΝ διαιρείται με το ${q1Div}.`
    },
    q2: {
      type: 'input',
