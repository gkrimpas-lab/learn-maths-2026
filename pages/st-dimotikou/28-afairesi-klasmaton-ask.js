...
const REAL_WORLD_SCENARIOS = [
  { item: 'της πίτσας', initialText: 'Είχαμε τα', usedText: 'και φάγαμε τα', remText: 'περίσσεψε' },
  { item: 'του χυμού', initialText: 'Το μπουκάλι είχε τα', usedText: 'και ήπιαμε το', remText: 'έμεινε' },
  { item: 'του υφάσματος', initialText: 'Το τόπι είχε τα', usedText: 'και κόψαμε το', remText: 'περίσσεψε' },
  { item: 'του κήπου', initialText: 'Είχαμε σκάψει τα', usedText: 'και φυτέψαμε τα', remText: 'έμεινε άσκαφτο' },
  { item: 'της σοκολάτας', initialText: 'Η πλάκα είχε τα', usedText: 'και μοιράσαμε τα', remText: 'περίσσεψε' },
  { item: 'του χαρτιού', initialText: 'Είχαμε τα', usedText: 'και χρησιμοποιήσαμε τα', remText: 'έμεινε' },
  { item: 'της μπογιάς', initialText: 'Το δοχείο είχε τα', usedText: 'και ξοδέψαμε τα', remText: 'περίσσεψε' },
  { item: 'της διαδρομής', initialText: 'Είχαμε σχεδιάσει τα', usedText: 'και διανύσαμε τα', remText: 'απομένει' }
];
...
function generateQuestions() {
  ...
  const scD1 = [4, 6, 8, 10][getRandomInt(0, 3)];
  const scN1 = getRandomInt(Math.floor(scD1 / 2) + 1, scD1 - 1);
  const scD2 = [2, 3, 4, 5][getRandomInt(0, 3)];
  const scN2 = 1; // διασφαλίζει πάντα θετικό αποτέλεσμα scN1/scD1 > scN2/scD2

  const scLcm = findLCM(scD1, scD2);
  const scEquiv1 = scN1 * (scLcm / scD1);
  const scEquiv2 = scN2 * (scLcm / scD2);
  const scDiffN = scEquiv1 - scEquiv2;
  const scG = findGCD(scDiffN, scLcm);
  const scCorrectStr = scG > 1 && (scDiffN / scG !== scDiffN)
    ? `${scDiffN / scG}/${scLcm / scG}`
    : `${scDiffN}/${scLcm}`;

  const scPrompt = `${sc.initialText} ${scN1}/${scD1} ${sc.item} ${sc.usedText} ${scN2}/${scD2} ${sc.item}. Ποιο μέρος ${sc.remText};`;
  const scWrongs = [
    `${scDiffN + 1}/${scLcm}`,
    `${Math.max(1, scN1 - scN2)}/${scLcm + 1}`,
  ...
  return {
    ...
    q5: {
      ...
      correct: q5IsTrue,
      explain: q5IsTrue
        ? 'Στα ομώνυμα κλάσματα αφαιρούμε ΜΟΝΟ τους αριθμητές και κρατάμε τον ίδιο παρονομαστή.'
        : 'ΠΟΤΕ δεν αφαιρούμε τους παρονομαστές μεταξύ τους.'
    },
    q6: {
      type: 'tf',
      ...
      correct: q6IsTrue,
      explain: q6IsTrue
        ? 'Για να αφαιρέσουμε ετερώνυμα κλάσματα πρέπει πρώτα να τα κάνουμε ομώνυμα με το Ε.Κ.Π.'
        : 'Δεν μπορούμε να αφαιρέσουμε ετερώνυμα κλάσματα χωρίς να τα μετατρέψουμε πρώτα σε ομώνυμα.'
    },
    q7: {
      type: 'input',
    ...
    q8: {
      ...
      explain: `Κάνουμε ομώνυμα και αφαιρούμε: ${scEquiv1}/${scLcm} － ${scEquiv2}/${scLcm} ＝ ${scDiffN}/${scLcm}${scG > 1 ? ` ＝ ${scCorrectStr}` : ''}.`
    }
  };
}
...
export default function AfairesiKlasmatonExercisesPage() {
  ...
  return (
    <Layout
      title="🎯 Ασκήσεις: 28. Αφαίρεση Κλασμάτων - ΣΤ' Δημοτικού | LearnMaths.gr"
      description="Διαδραστικές ασκήσεις με αυτόματη βαθμολόγηση στην αφαίρεση ομώνυμων και ετερώνυμων κλασμάτων για τη ΣΤ' Δημοτικού."
