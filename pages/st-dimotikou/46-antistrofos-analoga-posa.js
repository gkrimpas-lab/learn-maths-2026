            ...
            <div className="lg:col-span-7 bg-slate-50 p-6 rounded-3xl border border-slate-200 flex flex-col items-center justify-center">
              ...
              <div className="w-full max-w-[420px] aspect-square bg-white rounded-2xl border border-slate-200 p-4 relative shadow-inner">
                <svg viewBox="0 0 400 400" className="w-full h-full overflow-visible">
                  ...
                  {/* Ετικετες αξονων */}
                  <text x="365" y="375" fontSize="11" fontWeight="bold" fill="#64748b" textAnchor="end">Ταχύτητα υ (km/h)</text>
                  ...
                  {/* Καμπυλη Υπερβολης: t = 240 / v (απο v=20 εως v=130) */}
                  {/* Mapping: x = 50 + (v / 120) * 300, y = 350 - (t / 8) * 280 */}
                  <path
                    d="M 100 70 Q 125 180 200 280 T 350 315"
                    fill="none"
                    stroke="#e11d48"
                    strokeWidth="3.5"
                    strokeLinecap="round"
                  ...
                  {/* Ενεργο σημειο πανω στην καμπυλη */}
                  {/* pixel X: 50 + (speedVal / 120) * 300 */}
                  {/* pixel Y: 350 - (tripTime / 8) * 280 */}
                  <circle
                    cx={50 + (speedVal / 120) * 300}
                    cy={Math.max(50, Math.min(345, 350 - (tripTime / 8) * 280))}
                    r="7"
                    fill="#2563eb"
                    stroke="#ffffff"
                    strokeWidth="2"
