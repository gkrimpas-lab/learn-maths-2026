...
export default function DekadikoiArithmoiPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col justify-between">
        ...
        <section className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 text-white py-12 px-4 shadow-inner">
          <div className={`${LAYOUT.CONTAINER} grid grid-cols-1 lg:grid-cols-12 gap-8 items-center`}>
            ...
            <div className="lg:col-span-4 bg-white/10 backdrop-blur-md border border-white/25 rounded-3xl p-6 text-center space-y-4 shadow-xl">
              ...
              <Link 
                href="/st-dimotikou/02-dekadikoi-ask" 
                className="block w-full py-3 px-4 bg-amber-400 hover:bg-amber-300 text-slate-900 font-extrabold rounded-2xl transition duration-200 shadow-lg text-center"
              >
        ...
        <main className={`${LAYOUT.LESSON_CONTAINER} py-12 space-y-12`}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-emerald-50/70 border border-emerald-100 p-6 rounded-3xl space-y-3 shadow-sm hover:shadow-md transition">
              ...
              <h2 className="text-lg font-black text-slate-900">Ακέραιο & Δεκαδικό Μέρος</h2>
              ...
              <div className="bg-white p-3 rounded-xl border border-emerald-100 text-xs font-mono text-center text-slate-700">
            ...
            <div className="bg-blue-50/70 border border-blue-100 p-6 rounded-3xl space-y-3 shadow-sm hover:shadow-md transition">
              ...
              <h2 className="text-lg font-black text-slate-900">Τάξεις Δεκαδικού Μέρους</h2>
              <p className="text-sm text-slate-600 leading-relaxed">
                Κάθε θέση δεξιά από την υποδιαστολή υποδηλώνει ένα δεκαδικό κλάσμα:
              </p>
              <ul className="text-xs text-slate-700 space-y-1 font-medium bg-white p-3 rounded-xl border border-blue-100">
            ...
            <div className="bg-amber-50/70 border border-amber-100 p-6 rounded-3xl space-y-3 shadow-sm hover:shadow-md transition">
              ...
              <h2 className="text-lg font-black text-slate-900">Ισοδύναμοι Δεκαδικοί</h2>
              <p className="text-sm text-slate-600 leading-relaxed">
                Αν προσθέσουμε ή αφαιρέσουμε <strong>μηδενικά στο τέλος</strong> του δεκαδικού μέρους, η αξία του αριθμού δεν αλλάζει!
              </p>
              <div className="bg-white p-3 rounded-xl border border-amber-100 text-xs text-slate-700 font-bold text-center">
                3,5 = 3,50 = 3,500
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
              ...
              <div className="lg:col-span-7 flex flex-col justify-between space-y-6">
                ...
                <div className="overflow-x-auto pb-2">
                  <div className="min-w-[500px] bg-white rounded-2xl border border-slate-300 shadow-sm overflow-hidden">
                    ...
                    <div className="grid grid-cols-7 text-[10px] font-black text-slate-500 text-center border-b bg-slate-100 uppercase py-2">
                      {intClasses.map((c, i) => (
                        <div key={`hc1-${i}`} className="border-r border-slate-200" title={c.name}>
                          {c.label}
                        </div>
                      ))}
                      <div className="text-amber-600 font-bold border-r border-slate-200 bg-amber-50/50">Υποδ.</div>
                      {decClasses.map((c, i) => (
                        <div key={`hc2-${i}`} className="border-r border-slate-200 last:border-0" title={c.name}>
                          {c.label}
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-7 text-center items-center font-black text-2xl">
                      ...
                      {intDigits.map((digit, i) => {
                        ...
                        return (
                          <div 
                            key={`id-${i}`} 
                            className={`py-5 border-r border-slate-200 bg-emerald-50/40 transition-colors duration-200 ${
                      ...
                      {/* Υποδιαστολή */}
                      <div className="py-5 border-r border-slate-200 bg-amber-50 text-amber-500 font-black">
                        ,
                      </div>
                      ...
                      {decDigits.map((digit, i) => {
                        ...
                        return (
                          <div 
                            key={`dd-${i}`} 
                            className={`py-5 border-r border-slate-200 last:border-0 bg-blue-50/40 transition-colors duration-200 ${
                ...
                {/* SVG Οπτικοποίηση - Μπάρες Κατανομής */}
              ...
              <div className="lg:col-span-5 flex flex-col justify-between space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {presets.map((item, idx) => (
                        <button
                          ...
                          className="text-left px-3 py-2 bg-white hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 rounded-xl text-xs font-bold text-slate-700 transition shadow-sm"
                        >
                          {item.label}
                        </button>
                      ))}
                ...
                {/* Αναλυτική Μορφή σε Κλάσματα ΧΩΡΙΣ Scrollbar */}
                <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl flex-1 flex flex-col justify-between shadow-inner">
                  <div>
                    ...
                    <div className="space-y-2.5 font-mono text-xs md:text-sm">
                      ...
                      {intDigits.map((digit, i) => {
                        ...
                        return (
                          <div key={`int-row-${i}`} className="flex items-center justify-between py-1.5 border-b border-slate-200/70">
                            ...
                            <span className="text-xs text-slate-500 bg-white px-2 py-0.5 rounded-md border border-slate-200 font-sans font-bold">
                              = {Number(digit) * weight}
                            </span>
                          </div>
                        );
                      ...
                      {decDigits.map((digit, i) => {
                        ...
                        return (
                          <div key={`dec-row-${i}`} className="flex items-center justify-between py-1.5 border-b border-slate-200/70 last:border-0">
                            <span className="flex items-center gap-1.5">
                  ...
                  <div className="pt-4 mt-4 border-t border-slate-200 flex justify-between items-center text-xs">
                    ...
                    <strong className="text-base text-slate-900 font-black">{displayFormatted || "0"}</strong>
                  </div>
                </div>
      ...
      {/* 5. FOOTER */}
      <footer className="bg-slate-900 text-slate-400 py-8 text-center text-sm w-full border-t border-slate-800 mt-16">
        <div className={LAYOUT.CONTAINER}>
          <p className="font-medium">© 2026 LearnMaths.gr — Διαδραστική Εκμάθηση Μαθηματικών Δημοτικού & Γυμνασίου.</p>
        </div>
      </footer>
    </div>
  );
