...
function generateQuestions() {
  ...
  // Q1: Input - Διαίρεση με 10, 100, 1000
  const q1Int = getRandomInt(12, 95);
  const q1Dec = getRandomInt(1, 9); // π.χ. 45,2
  const q1Div = [10, 100, 1000][getRandomInt(0, 2)];
  ...
  const q2Int = getRandomInt(2, 48);
  const q2Dec = getRandomInt(1, 9); // π.χ. 3,5
  const q2Div = [0.1, 0.01, 0.001][getRandomInt(0, 2)];
  ...
  const q2DivStr = q2Div.toString().replace('.', ',');
  const q2Prompt = `${q2Int},${q2Dec} : ${q2DivStr}`;

  // Q3: MCQ - Εύρεση του διαιρέτη που λείπει (π.χ. 45,2 : ? = 0,452)
  const q3Int = getRandomInt(15, 85);
  const q3Dec = getRandomInt(1, 9);
  const q3ValStr = `${q3Int},${q3Dec}`;
  const q3DivType = getRandomInt(1, 4); // 1: :10, 2: :100, 3: :1000, 4: :0.1
  let q3CorrectDiv = "100";
  let q3ResultStr = "";

  if (q3DivType === 1) {
    q3CorrectDiv = "10";
    q3ResultStr = (parseFloat(`${q3Int}.${q3Dec}`) / 10).toFixed(2).replace('.', ',');
  } else if (q3DivType === 2) {
    q3CorrectDiv = "100";
    q3ResultStr = (parseFloat(`${q3Int}.${q3Dec}`) / 100).toFixed(3).replace('.', ',');
  } else if (q3DivType === 3) {
    q3CorrectDiv = "1000";
    q3ResultStr = (parseFloat(`${q3Int}.${q3Dec}`) / 1000).toFixed(4).replace('.', ',');
  } else {
    q3CorrectDiv = "0,1";
    q3ResultStr = (parseFloat(`${q3Int}.${q3Dec}`) / 0.1).toFixed(1).replace('.', ',');
  }
  const q3Options = shuffle(["10", "100", "1000", "0,1", "0,01"].filter(d => d !== q3CorrectDiv).slice(0, 3).concat(q3CorrectDiv));

  // Q4: MCQ - Πρόβλημα Καθημερινότητας (π.χ. Μοιράζουμε σε 100 ίσα μέρη)
  const q4Item = shuffledItems[0];
  const q4Val = parseFloat((q4Item.total / q4Item.group).toFixed(2));
  const q4Correct = `${q4Val.toString().replace('.', ',')} ${q4Item.unit}`;
  const q4Wrong1 = `${(q4Val * 10).toString().replace('.', ',')} ${q4Item.unit}`;
  const q4Wrong2 = `${(q4Val / 10).toString().replace('.', ',')} ${q4Item.unit}`;
  const q4Wrong3 = `${(q4Item.total + q4Item.group).toString().replace('.', ',')} ${q4Item.unit}`;
  const q4Options = shuffle([q4Correct, q4Wrong1, q4Wrong2, q4Wrong3]);

  // Q5: True / False - Κανόνας μετατόπισης αριστερά (10, 100, 1000)
  ...
  // Q6: True / False - Κανόνας μετατόπισης δεξιά (0,1, 0,01) & Ισοδυναμία
  ...
  const q7Int = getRandomInt(12, 85);
  const q7Dec = getRandomInt(1, 9);
  const q7Div = [10, 100][getRandomInt(0, 1)];
  const q7Val = parseFloat(`${q7Int}.${q7Dec}`);
  const q7Ans = (q7Val / q7Div).toString().replace('.', ',');

  // Q8: SVG Visual - Αναγνώριση Πράξης από Μετατόπιση (MCQ)
  const q8BaseInt = getRandomInt(35, 95);
  ...
  const q8Options = shuffle([": 0,1", ": 10", ": 0,01", ": 100"]);

  return {
    q1: {
      type: 'input',
      title: 'Διαίρεση με 10, 100, 1000',
      prompt: q1Prompt,
      correct: q1Correct,
      explain: `Διαιρώντας με το ${q1Div}, μετακινούμε την υποδιαστολή ${q1Div === 10 ? '1 θέση' : q1Div === 100 ? '2 θέσεις' : '3 θέσεις'} προς τα αριστερά: ${q1Prompt} ＝ ${q1Correct}.`
    },
    q2: {
      type: 'input',
      title: 'Διαίρεση με 0,1, 0,01, 0,001',
      prompt: q2Prompt,
      correct: q2Correct,
      explain: `Διαιρώντας με το ${q2DivStr}, μετακινούμε την υποδιαστολή ${q2Div === 0.1 ? '1 θέση' : q2Div === 0.01 ? '2 θέσεις' : '3 θέσεις'} προς τα δεξιά: ${q2Prompt} ＝ ${q2Correct}.`
    },
    q3: {
      type: 'mcq',
      title: 'Εύρεση Διαιρέτη',
      prompt: `Βρες τον διαιρέτη που λείπει: ${q3ValStr} : ? ＝ ${q3ResultStr}`,
      options: q3Options,
      correct: q3CorrectDiv,
      explain: `Για να φτάσουμε από το ${q3ValStr} στο ${q3ResultStr}, η υποδιαστολή μετακινήθηκε κατάλληλα. Ο σωστός διαιρέτης είναι το ${q3CorrectDiv}.`
    },
    q4: {
      type: 'mcq',
      ...
      options: q4Options,
      correct: q4Correct,
      explain: `Υπολογίζουμε τη διαίρεση: ${q4Item.total} : ${q4Item.group} ＝ ${q4Correct}.`
    },
    q5: {
      type: 'tf',
      ...
      correct: q5IsTrue,
      explain: q5IsTrue
        ? 'Σωστά! Η διαίρεση με 10, 100, 1000 μικραίνει τον αριθμό, άρα η υποδιαστολή μετακινείται αριστερά.'
    ...
    q6: {
      ...
      correct: q6IsTrue,
      explain: q6IsTrue
        ? 'Σωστά! Η διαίρεση με το 0,1 ισοδυναμεί με πολλαπλασιασμό επί 10, επομένως ο αριθμός μεγαλώνει.'
    ...
    q7: {
      ...
      div: q7Div,
      correct: q7Ans,
      explain: `Ξεκινώντας από το ${q7Int},${q7Dec} και κάνοντας ${q7Div === 10 ? '1 άλμα' : '2 άλματα'} αριστερά λόγω του :${q7Div}, βρίσκουμε ${q7Ans}.`
    },
    q8: {
      type: 'mcq',
      ...
      options: q8Options,
      correct: q8CorrectDiv,
      explain: `Η υποδιαστολή μετακινήθηκε 1 θέση προς τα δεξιά (από ${q8StartStr} σε ${q8TargetStr}), άρα η πράξη είναι ${q8CorrectDiv}.`
    }
  };
}
...
export default function DiairesiDinameisDekaExercisesPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col justify-between pb-32">
      <Head>
        <title>🎯 Ασκήσεις: Διαίρεση με Δυνάμεις του 10 - ΣΤ' Δημοτικού | LearnMaths.gr</title>
        <meta name="description" content="Διαδραστικές ασκήσεις με αυτόματη βαθμολόγηση στη διαίρεση με δυνάμεις του 10 για τη ΣΤ' Δημοτικού." />
          ...
          <div className={`${LAYOUT.CONTAINER} flex flex-col md:flex-row justify-between items-center gap-6`}>
            <div className="space-y-2 text-center md:text-left">
              ...
              <h1 className="text-3xl md:text-4xl font-black tracking-tight">
                Διαδραστικές Ασκήσεις: Διαίρεση με Δυνάμεις του 10
              </h1>
        ...
        <main className={`${LAYOUT.LESSON_CONTAINER} py-10`}>
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* ΕΡΩΤΗΣΗ 7: Οπτικό SVG (Μετατόπιση Αριστερά) */}
              <div className={`p-6 rounded-3xl border transition-all ${getCardStyle('q7')}`}>
                ...
                <p className="text-sm text-slate-700 mb-3 font-medium">
                  Υπολόγισε το αποτέλεσμα της μετατόπισης: <strong className="text-rose-700 font-mono">{questions.q7.startStr} : {questions.q7.div}</strong>
                </p>
                
                {/* SVG Jump Arc */}
                <div className="bg-slate-100 rounded-2xl p-4 mb-4 flex justify-center">
                  <svg viewBox="0 0 280 60" className="w-full max-w-xs h-14">
                    <text x="200" y="45" fontSize="22" fontWeight="black" fill="#1e293b" fontFamily="monospace">
                      {questions.q7.startStr}
                    </text>
                    <path
                      d="M 200 25 Q 150 5 100 25"
                      fill="none"
                      stroke="#f59e0b"
                      strokeWidth="3"
                    />
                    <polygon points="100,25 110,18 107,26" fill="#f59e0b" />
                    <text x="150" y="15" fontSize="10" fontWeight="black" textAnchor="middle" fill="#d97706">
                      : {questions.q7.div}
                    </text>
                  </svg>
                </div>
              ...
              <div className={`p-6 rounded-3xl border transition-all ${getCardStyle('q8')}`}>
                ...
                <div className="bg-slate-100 p-3 rounded-2xl mb-3 flex items-center justify-center">
                  <div className="flex items-center gap-3 font-mono font-black text-lg">
                    <span className="bg-white px-3 py-1.5 rounded-xl border border-slate-300 text-slate-800">{questions.q8.startStr}</span>
                    <span className="text-amber-500 font-sans">➔</span>
                    <span className="bg-teal-50 px-3 py-1.5 rounded-xl border border-teal-300 text-teal-800">{questions.q8.targetStr}</span>
                  </div>
                </div>
