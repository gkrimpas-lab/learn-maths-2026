...
const LIMITS = {
  NUM_MIN: 2,
  NUM_MAX: 12,
  COUNT_SHOW: 12 // Πόσα πολλαπλάσια δείχνουμε ανά αριθμό
};
...
export default function EkpPage() {
  ...
  // Συναρτήσεις υπολογισμού ΜΚΔ και ΕΚΠ για δύο αριθμούς
  const findGCD = (a, b) => {
    while (b) {
      let t = b;
      b = a % b;
  ...
  const findLCMOfTwo = (a, b) => (a * b) / findGCD(a, b);
  ...
  // Παραγωγή λιστών (πολλαπλάσια από το 1 έως το 12, παραλείποντας το 0)
        ...
        <main className={`${LAYOUT.LESSON_CONTAINER} py-12`}>
          <div className="space-y-8 bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="bg-gray-50 p-6 md:p-8 rounded-2xl border border-gray-200 space-y-8">
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-wrap justify-center items-center gap-6 max-w-4xl mx-auto">
                {/* Αριθμός 1 */}
                <div className="flex items-center gap-2">
                  <span className="font-bold text-gray-500 text-xs">1ος:</span>
                  <button onClick={() => setN1(Math.max(LIMITS.NUM_MIN, n1 - 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">-</button>
                  <span className="w-6 text-center font-black text-base text-blue-600">{n1}</span>
                  <button onClick={() => setN1(Math.min(LIMITS.NUM_MAX, n1 + 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">+</button>
                </div>

                {/* Αριθμός 2 */}
                <div className="flex items-center gap-2">
                  <span className="font-bold text-gray-500 text-xs">2ος:</span>
                  <button onClick={() => setN2(Math.max(LIMITS.NUM_MIN, n2 - 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">-</button>
                  <span className="w-6 text-center font-black text-base text-indigo-600">{n2}</span>
                  <button onClick={() => setN2(Math.min(LIMITS.NUM_MAX, n2 + 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">+</button>
                </div>

                {/* Αριθμός 3 (Δυναμικός) */}
                {numCount >= 3 && (
                  <div className="flex items-center gap-2 animate-fade-in">
                    <span className="font-bold text-gray-500 text-xs">3ος:</span>
                    <button onClick={() => setN3(Math.max(LIMITS.NUM_MIN, n3 - 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">-</button>
                    <span className="w-6 text-center font-black text-base text-purple-600">{n3}</span>
                    <button onClick={() => setN3(Math.min(LIMITS.NUM_MAX, n3 + 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">+</button>
                  </div>
                )}
                ...
                {numCount === 4 && (
                  <div className="flex items-center gap-2 animate-fade-in">
                    <span className="font-bold text-gray-500 text-xs">4ος:</span>
                    <button onClick={() => setN4(Math.max(LIMITS.NUM_MIN, n4 - 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">-</button>
                    <span className="w-6 text-center font-black text-base text-emerald-600">{n4}</span>
                    <button onClick={() => setN4(Math.min(LIMITS.NUM_MAX, n4 + 1))} className="bg-slate-200 w-8 h-8 rounded-full font-black hover:bg-slate-300">+</button>
                  </div>
                )}
              </div>
